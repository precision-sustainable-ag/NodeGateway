...
/// For Macronix MX25R6434F: C217 (page 25: http://www.macronix.com/Lists/Datasheet/Attachments/7428/MX25R6435F,%20Wide%20Range,%2064Mb,%20v1.4.pdf)
...
boolean SPIFlash::initialize()
{
  ...
  if (_jedecID == 0 || readDeviceId() == _jedecID) {
    ...
    SPI.transfer(0);                     // Global Unprotect --> WHAT IS THIS DOING?
    unselect();
    return true;
  }
  return false;
...
uint8_t SPIFlash::readStatus()
{
  select();
  SPI.transfer(SPIFLASH_STATUSREAD);
  uint8_t status = SPI.transfer(0);  // WIP (Write in Progress bit) 1 = writing (busy), 0 = not writing (not busy)
  unselect();
  return status;
}
...
/// write multiple bytes to flash memory (up to 64K = block)
/// WARNING: you can only write to previously erased memory locations (see datasheet)
///          use the block erase commands to first clear memory (write 0xFFs)
...
void SPIFlash::writeBytes(uint32_t addr, const void* buf, uint16_t len) {
  ...
  while (len>0)
  {
    ...
    command(SPIFLASH_BYTEPAGEPROGRAM, true);  // Byte/Page Program
    SPI.transfer(addr >> 16);     // send address
    SPI.transfer(addr >> 8);
    SPI.transfer(addr);

    for (uint16_t i = 0; i < n; i++)
      SPI.transfer(((uint8_t*) buf)[offset + i]);  // write first 256 bytes
    unselect();
...
void SPIFlash::sectorErase4K(uint32_t addr) {
  command(SPIFLASH_SECTORERASE_4K, true); // Block Erase
  SPI.transfer(addr >> 16);
  SPI.transfer(addr >> 8);
  SPI.transfer(addr);
  unselect();
